...
module.exports = class ContactAddress {
    ...
    toArray(){
        return [  Buffer.from(this.protocol, "ascii"), Buffer.from(this.hostname, "ascii"), this.port, Buffer.from(this.path, "ascii") ];
    }

    static fromArray(arr, offset ){
        return new ContactAddress( arr[offset].toString(), arr[offset+1].toString(), arr[offset+2], arr[offset+3].toString() );
    }
